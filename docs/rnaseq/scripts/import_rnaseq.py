...
Usage:
    python import_rnaseq.py <metadata.xlsx> [--output-dir <dir>] [--min-alignment <pct>]

Examples:
    python import_rnaseq.py Shivarathri_2022_metadata.xlsx --output-dir ./output
    python import_rnaseq.py Iracane_2024_sRNA.xlsx --min-alignment 65  # Override 85% threshold
"""
...
DEFAULT_ALIGNMENT_THRESHOLD = 85.0  # Minimum alignment percentage
...
def parse_metadata(filepath: str, alignment_threshold: float = DEFAULT_ALIGNMENT_THRESHOLD) -> tuple[dict, list[dict], list[dict]]:
    """Parse metadata file (XLSX or TSV).

    Args:
        filepath: Path to metadata file
        alignment_threshold: Minimum alignment percentage (default: 85.0)
...
def generate_failed_report(study_info: dict, failed_samples: list[dict], alignment_threshold: float = DEFAULT_ALIGNMENT_THRESHOLD) -> str:
...
Examples:
  ...
  python import_rnaseq.py Iracane_2024_sRNA.xlsx --min-alignment 65
        '''
    )
    ...
    parser.add_argument('--min-alignment', type=float, default=DEFAULT_ALIGNMENT_THRESHOLD,
                        help=f'Minimum alignment percentage threshold (default: {DEFAULT_ALIGNMENT_THRESHOLD})')

    args = parser.parse_args()
    alignment_threshold = args.min_alignment

    # Parse metadata
    print(f"Reading metadata from: {args.metadata_file}")
    if alignment_threshold != DEFAULT_ALIGNMENT_THRESHOLD:
        print(f"Using custom alignment threshold: {alignment_threshold}%")
    study_info, passed_samples, failed_samples = parse_metadata(args.metadata_file, alignment_threshold)
    ...
    if failed_samples:
        failed_report = generate_failed_report(study_info, failed_samples, alignment_threshold)
