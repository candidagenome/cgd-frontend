  ...
  return (
    <div className="go-details">
      {organisms.map(([orgName, orgData]) => {
        const grouped = groupByAspect(orgData.annotations || []);
          ...
          <div key={orgName} className="organism-section">
            ...
            {orgData.annotations && orgData.annotations.length > 0 ? (
              <div className="go-aspects">
                {Object.entries(grouped).map(([aspect, data]) => {
                  if (data.annotations.length === 0) return null;

                  const sectionKey = `${orgName}-${aspect}`;
                  ...
                  return (
                    <div key={aspect} className="aspect-section">
                      ...
                      >
                        ...
                        <span className="count-badge">{data.annotations.length}</span>
                      </h4>

                      {!isCollapsed && (
                        <div className="aspect-content">
                          {/* Show annotations grouped by evidence category */}
                          {Object.entries(data.byEvidence).map(([category, annotations]) => (
