              ...
              {ann.qualifier && (
                <span
                  className={`go-qualifier ${
                    ann.qualifier.toLowerCase() === 'not' ? 'qualifier-not' : ''
                  }`}
                >
                  {ann.qualifier}
                </span>
        ...
        cellRenderer: (params) => {
          ...
          return (
            <div className="go-evidence-cell">
              ...
              {ann.date_created && <div className="assigned-date">Assigned on {ann.date_created}</div>}
            </div>
          );
        },
      },
  ...
  return (
    <div className="go-details">
      ...
      <div className="go-intro">
        <p>
          This page displays GO annotations in different sections according to the methods used in the reference from
          which the annotation was made:
        </p>
        <ul>
          <li>
            <a href="#manually-curated">
              <strong>Manually Curated GO Annotations</strong>
            </a>
            : includes annotations based on published experiments or analyses that focus on specific genes.
          </li>
          <li>
            <a href="#high-throughput">
              <strong>GO Annotations from High-throughput Experiments</strong>
            </a>
            : includes annotations made from published experiments performed on a high-throughput or genome-wide basis.
          </li>
          <li>
            <a href="#computational">
              <strong>Computational GO Annotations</strong>
            </a>
            : includes annotations that are predicted by computational methods (e.g., sequence similarity comparisons)
            and are not individually reviewed.
          </li>
        </ul>
      </div>
            ...
            <div className="go-annotation-types">
              {Object.entries(ANNOTATION_TYPE_LABELS).map(([typeKey, typeLabel]) => {
                ...
                const lastReviewedDate =
                  typeKey === 'manually curated' && orgData.last_reviewed_date
                ...
                if (totalCount === 0) {
                  return (
                    <div key={typeKey} id={typeKey.replace(/\s+/g, '-')} className="annotation-type-section">
                      ...
                      <p className="no-data">
                        No {typeLabel} GO Annotations for {orgData.locus_display_name}.
                      </p>
                    </div>
                  );
                }
