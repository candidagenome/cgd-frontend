...
function GoTermPage() {
  ...
  const geneColumnDefs = useMemo(
    () => [
      ...
      },
      {
        ...
        cellRenderer: (params) => <em>{getOrganismAbbrev(params.data.species)}</em>,
      },
      {
  ...
  const getAmigoUrl = (goidVal) => {
    return `http://amigo.geneontology.org/amigo/term/${goidVal}`;
  };

  // Format GOID for display (ensure GO:XXXXXXX format)
  const formatGoid = (goidVal) => {
    if (!goidVal) return '';
    if (goidVal.startsWith('GO:')) return goidVal;
    return `GO:${goidVal.padStart(7, '0')}`;
  };
          ...
          <table className="info-table">
            <tbody>
              ...
              <tr>
                <th>GOID</th>
                <td>
                  <a href={getAmigoUrl(term.goid)} target="_blank" rel="noopener noreferrer">
                    {term.goid}
                  </a>
                </td>
              </tr>
      ...
      <div className="section" id="graphical-display">
        ...
        <div className="section-content">
          <p className="diagram-description">
            This diagram shows the local area of the Gene Ontology hierarchy centered on the term{' '}
            <strong>{data.term.go_term}</strong>. Parent terms (ancestors) are shown above the current term. Click any
            node to navigate to that term.
          </p>
          <GODiagram goid={goid} />
        </div>
      </div>
      ...
      <div className="section" id="additional-links">
        ...
        <div className="section-content">
          <ul className="external-links-list">
            ...
            <li>
              Search for Candida genes manually annotated to this term or to any manually annotated terms that are
              descended from this term, i.e., child terms representing more specific biology than this term.
            </li>
          </ul>
        </div>
      </div>
  ...
  const renderQualifierGroupTable = (annotationType, group) => {
    ...
    return (
      ...
      >
        <h4 className="qualifier-header">{group.display_name}</h4>
  ...
  const renderAnnotationTypeSection = (annotation) => {
    if (!annotation.qualifier_groups || annotation.qualifier_groups.length === 0) {
      return (
        <div key={annotation.annotation_type} className="annotation-type-section" id={annotation.annotation_type}>
          <h3 className="table-header">{ANNOTATION_TYPE_NAMES[annotation.annotation_type] || annotation.annotation_type}</h3>
    ...
    return (
      <div key={annotation.annotation_type} className="annotation-type-section" id={annotation.annotation_type}>
        ...
        {annotation.qualifier_groups.map((group) => renderQualifierGroupTable(annotation.annotation_type, group))}
      </div>
    );
  };
      ...
      <div className="section" id="annotations">
        ...
        <div className="section-content">
          <p className="annotation-note">
            The tables below show the genes that have been directly annotated to the term <strong>{data.term.go_term}</strong>{' '}
    ...
    return (
      <div className="go-term-page">
        <div className="loading-page">
          ...
          <p>
            Loading GO term information for <strong>{formatGoid(goid)}</strong>...
          </p>
        </div>
      </div>
    );
        ...
        <div className="error-page">
          ...
          <div className="error-suggestions">
            ...
            <ul>
              ...
              <li>
                Try searching for the GO term at{' '}
                <a href="http://amigo.geneontology.org/" target="_blank" rel="noopener noreferrer">
                  AmiGO
                </a>
              </li>
              <li>
                Use the <Link to="/go-resources">GO Resources</Link> page to explore GO tools
              </li>
            </ul>
          </div>
          <div className="error-actions">
            ...
            <Link to="/go-resources" className="btn-go">
              GO Resources
            </Link>
          </div>
        </div>
    ...
    <div className="go-term-page">
      <header className="go-term-header">
        ...
        <p className="subtitle">
          <a href={getAmigoUrl(data.term.goid)} target="_blank" rel="noopener noreferrer">
            {data.term.goid}
          </a>
          {' - '}
